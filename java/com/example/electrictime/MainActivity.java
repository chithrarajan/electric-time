...
public class MainActivity extends AppCompatActivity {
    ...
    public void doWalk(View v) {
        ...
        if (getNumbers()) {
            if (num1 > 30) {
                String outOfBounds = "The distance inputted is out of range.";
                t1.setText(outOfBounds);
            }
            else {
                double time = ( 60 / 3.1) * num1;
                if (time >= 60) {
                    time = time/ 60;
                    t1.setText(Double.toString(time)+"hours");
                }
                else {
                    t1.setText(Double.toString(time)+"minutes");
                }
            }
        }
    }
